...
- **Dev-only column add for SQLite.** `create_all` is no-op on existing tables. Added `_dev_alter_table_for_existing_sqlite()` in `database.py` that runs after `create_all`, reads `PRAGMA table_info(campaigns)`, and `ALTER TABLE` for any missing columns. SQLite-only, idempotent, drop-able when we move to Postgres + Alembic in Session 16. Without this, every column-adding session would force a volume reset.
