...
async def proof(
    ...
    # 4. Atomic nonce claim — first writer wins, duplicates get 409
    try:
    ...
    # 6. Decrement budget before settling — nonce already claimed, so a retry
    ...
    # 7. Build + send USDC transfer via Privy. If this fails, we still persist a
    ...
    # 8. Success — record settlement row
    db.add(
        _settlement_row(
            campaign_id=campaign.id,
            nonce=claims.nonce,
