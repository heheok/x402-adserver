...
async def execute_settlement(
    ...
    Session 16.9: money is integer micro end-to-end. The atomic UPDATE guard
    is now exact integer comparison (`budget - spent >= :amount_micro`); the
    historical `+ 1e-9` epsilon is gone. The COMPLETED-flip rule is bit-exact
    `remaining < cost_per_play`.
    """
    ...
    # `budget - spent >= amount_micro` and increments `spent`, plus flips
    # status to COMPLETED in the same statement when the new remaining can't
    # fund another play at this CPM. `rowcount == 0` means either the
    # campaign doesn't exist, isn't ACTIVE, or has insufficient budget — we
    # re-read to disambiguate for a useful error message.
    #
    ...
    new_spent = Campaign.spent + claims.amount_micro
    play_cost = Campaign.cpm_price / 1000  # int floor in SQL — exact
    stmt = (
        update(Campaign)
        .where(Campaign.id == claims.campaign_id)
        .where(Campaign.status == CampaignStatus.ACTIVE.value)
        .where(Campaign.budget - Campaign.spent >= claims.amount_micro)
        .values(
            spent=new_spent,
            status=case(
                (
                    Campaign.budget - new_spent < play_cost,
    ...
    row = _settlement_row(
        ...
        amount_micro=claims.amount_micro,
        tx_hash=None,
...
async def proof(
    ...
    if age < -60:  # modest skew tolerance (clock-skew, not money-drift)
